...
def test_typing():
    ...
    for dtype in [np.float16, np.float32, np.double, np.float64, np.float128]:
        ...
        dist_mat = dii._compute_full_dist_matrix(data.astype(dtype), period=np.zeros(5, dtype=dtype), njobs=1, cythond=True)
    ...
    assert cast_func(0, test=np.zeros(10, dtype=np.float16))[1]['test'].dtype == dii.CYTHON_DTYPE
    ...
    for n_data in np.logspace(1, 2, 10, dtype=np.int16):
        for n_dim in np.logspace(1, 2, 10, dtype=np.int16):
            ...
            for period, cythond in it.product(
                ...
                dist_mat = dii._compute_full_dist_matrix(data=data, period=period, cythond=cythond, njobs=1)
    ...
    for n_data in np.logspace(1, 2, 10, dtype=np.int16):
        for n_dim in np.logspace(1, 2, 10, dtype=np.int16):
            ...
            for cythond in [False, True]:
                ranks = dii._compute_full_rank_matrix(data=data, period=np.zeros((data.shape[-1]), dtype=np.double), cythond=cythond, njobs=1)
...
def test_py_kernel_gradient():
    for n_data in np.logspace(1, 2, 10, dtype=np.int16):
        for n_dim in np.logspace(1, 2, 10, dtype=np.int16):
            ...
            dist_mat = dii._compute_full_dist_matrix(data*gammas, period=np.zeros(n_dim, dtype=dii.CYTHON_DTYPE), njobs=njobs, cythond=False)
            ranks = dii._compute_full_rank_matrix(data, period=np.zeros(n_dim, dtype=dii.CYTHON_DTYPE), njobs=njobs, cythond=False)

            # alphagamma_gradientterm(data_A=data, dists_rescaled_A=dist_mat, alpha_gamma=0, gammas=gammas, period=np.zeros((n_dim)), njobs=njobs, cythond=True)
            # raise ValueError()
            py_grad = dii._compute_kernel_imbalance_gradient(
                dists_rescaled_A=dist_mat, data_A=data*gammas, rank_matrix_B=ranks, 
                gammas=gammas, lambd=1., period=np.zeros(n_dim, dtype=dii.CYTHON_DTYPE), njobs=njobs, cythond=False
            )
            # c_dii.compute_kernel_imbalance_gradient_cython(
            #     dists_rescaled_A, data_A, rank_matrix_B,
            #     gammas, lambd, period, njobs, periodic = False
            # )
            cython_grad = c_dii.compute_kernel_imbalance_gradient_cython(
                dist_mat, data*gammas, ranks, 
                gammas, 1., np.zeros(n_dim, dtype=dii.CYTHON_DTYPE), njobs, False
            )
