...
class IdDiscrete(Base):
	...
	def __init__(self, coordinates=None, distances=None, maxk=None, weights=None, verbose=False, njobs=cores):
	...
	def fix_Lk(self,Lk=None,Ln=None,w=False):
		...
		assert self.distances is not None, 'first compute distances with the proper metric (manhattan of hamming presumably)' 

		if (Lk is not None and Ln is not None):
			self.set_Lk_Ln(Lk,Ln)
		else:
			assert (self.Lk is not None and self.Ln is not None), \
		...
		if w is False:
			...
			assert self.weights is not None,'first insert the weights if you want to use them!'
			self.k = np.array( [ sum( self.weights[ self.dist_indices[i][el] ] ) for i,el in enumerate(self.distances <= self.Lk) ], dtype=np.int )
			self.n = np.array( [ sum( self.weights[ self.dist_indices[i][el] ] ) for i,el in enumerate(self.distances <= self.Ln) ], dtype=np.int )
	...
	def compute_id_binomial_Lk(self,Lk=None,Ln=None, method='bayes',w=False):
		...
		if method == 'mle':
			if w:
				...
				ww = 1
			self.id_estimated_binom = SMin(    df.compute_binomial_logL, 10.,
	...
	def fix_k(self,k_eff=None,ratio=0.7):
		...
		if k_eff is not None:
			assert k_eff <= self.maxk, 'A k_eff > maxk was selected,\
		...
		# checks out
		if any(~self.mask):
			print('BE CAREFUL: '+ str(sum(~self.mask)) +' points would have Rk set to 0\
	...
	def set_w(self, w):
		assert ( len(w)==self.Nele and 
			all( [ wi > 0 and isinstance(wi,(np.int,int)) ] for wi in w ) 
