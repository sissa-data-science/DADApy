...
class Optimizer():
  ...
  def __init__(self,
              key=0,
              d0=0.,                    # BID
              d0_r=0.,                  # BID + random perturbation (*** used by compute_Pmodel instead of d0...)
              d1=0.,                    # slope
              d1_r=0.,                  # slope + random pertubation (*** used by compute_Pmodel instead of d1...)
              delta=0.,                 # optimization step size
              KL=jnp.double(0.),        # KL divergence between Pemp(r) and Pmodel(r)
              ...
              acc_ratio=jnp.double(1.), # Acceptance ratio
  ...
  def _tree_flatten(self):
    ...
    aux_data = {"Nsteps_max":self.Nsteps_max,
                }  # static values
...
register_pytree_node(Optimizer,
                    Optimizer._tree_flatten,
                    Optimizer._tree_unflatten
                    )
...
def compute_Pmodel(Op):
   Op = lax.fori_loop(lower = 0,
                    upper = Op.Pmodel.shape[0],
                    body_fun = _compute_Pmodel,
                    init_val = Op)
...
def step(idx,Op):
  ...
  Op.d0_r = Op.d0 * (1 + Op.delta * (r - jnp.double(.5)))

  Op.key, subkey = random.split(Op.key, num=2)
  rr = random.uniform(subkey, dtype=jnp.float64)
  Op.d1_r = Op.d1 * (1 + Op.delta * (rr - jnp.double(.5)))

  Op = compute_Pmodel(Op)
  Op = compute_KLd(Op)
  Op = lax.cond(Op.KL<=Op.KL_aux,
                update_state,
                do_nothing,
                Op)
  logical_condition = jnp.logical_and(
                        Op.save_logKLs_flag,
                        jnp.mod(idx,Op.mod_divisor)==0 
  )
  Op = lax.cond(logical_condition,
                save_logKL,
                do_nothing,
                Op)
  return Op
...
@jit
def minimize_KL(Op):
  Op.logKLs = jnp.empty(shape=(Op.Nsteps_max),
                        dtype=jnp.double)
...
class BID():
  ...
  def set_initial_condition(self,
    ...
    if self.L != 0:
      ...
      _d00_guess_list = self.L * jnp.arange(d00min,
                                            d00max+eps,
                                            d00step,
                                            dtype=jnp.double)
      _d10_guess_list = jnp.double(2) - _d00_guess_list / jnp.double(self.H.D_mu_emp)
    
      d00_guess_list = jnp.concatenate((d00_guess_list,
                                      _d00_guess_list)
                                    )
      d10_guess_list = np.concatenate((d10_guess_list,
                                      _d10_guess_list)
                                    )

    logKLs0 = jnp.empty(shape=(len(d00_guess_list)),
                        dtype=jnp.double)
    for i in range(len(d00_guess_list)):
      logKLs0 = logKLs0.at[i].set(test_initial_condition(self.Op,
                                                          d00_guess_list[i],
                                                          d10_guess_list[i],
                                                          )
                                  )
  ...
  def computeBID(self,):
    ...
    self.optimization_elapsed_time = (time() - starting_time) / 60. 
    print(f'optimization took {self.optimization_elapsed_time:.1f} minutes')
    print(f'd_0={self.Op.d0:.3f},d_1={self.Op.d1:.3f},logKL={jnp.log(self.Op.KL):.2f}')
    
    os.system(f'rm -f {self.optfile}')
    print(f'{self.rmax:d},{self.Op.d0:.8f},{self.Op.d1:8f},{np.log(self.Op.KL):.8f}',
            file=open(self.optfile, 'a'))
    np.savetxt(fname=self.valfile, 
