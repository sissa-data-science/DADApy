...
for i in reversed(range(0,nexp)):
    ...
    #kNN_Abr
    time_kNN_Abr_start = time.perf_counter()
    ksel_Abr[i]=Nsample[i]**(4./(4.+d))
    data.compute_density_kNN(ksel_Abr[i])
    time_kNN_Abr_end = time.perf_counter()
    time_kNN_Abr[i] = time_kNN_Abr_end - time_kNN_Abr_start

    off_k, F_k = _align_arrays(-data.log_den,data.log_den_err,F_anal_k)
    MAE_kNN_Abr[i] = np.mean(np.abs(F_k-F_anal_k))
    
    #kNN_Zhao
    time_kNN_Zhao_start = time.perf_counter()
    ksel_Zhao[i]=Nsample[i]**(2./(2.+d))
    data.compute_density_kNN(ksel_Zhao[i])
    time_kNN_Zhao_end = time.perf_counter()
    time_kNN_Zhao[i] = time_kNN_Zhao_end - time_kNN_Zhao_start
    off_k, F_k = _align_arrays(-data.log_den,data.log_den_err,F_anal_k)
    MAE_kNN_Zhao[i] = np.mean(np.abs(F_k-F_anal_k))
    
    #kstarNN
    time_kstarNN_start = time.perf_counter()
    data.compute_density_kstarNN()
    time_kstarNN_end = time.perf_counter()
    time_kstarNN[i] = time_kstarNN_end - time_kstarNN_start
    off_k, F_k = _align_arrays(-data.log_den,data.log_den_err,F_anal_k)
    MAE_kstarNN[i] = np.mean(np.abs(F_k-F_anal_k))

    #PAk
    time_PAk_start = time.perf_counter()
    data.compute_density_PAk()
    time_PAk_end = time.perf_counter()
    time_PAk[i] = time_PAk_end - time_PAk_start

    off_k, F_k = _align_arrays(-data.log_den,data.log_den_err,F_anal_k)
    MAE_PAk[i] = np.mean(np.abs(F_k-F_anal_k))

    #BMTI
    time_BMTI_start = time.perf_counter()
    data.compute_density_BMTI(comp_err=False)
    time_BMTI_end = time.perf_counter()
    time_BMTI[i] = time_BMTI_end - time_BMTI_start
    off_k, F_k = _align_arrays(-data.log_den,data.log_den_err,F_anal_k)
    MAE_BMTI[i] = np.mean(np.abs(F_k-F_anal_k))

    #GKDE Silverman
    time_GKDE_Sil_start = time.perf_counter()
    ...
    time_GKDE_Sil_end = time.perf_counter()
    time_GKDE_Sil[i] = time_GKDE_Sil_end - time_GKDE_Sil_start
